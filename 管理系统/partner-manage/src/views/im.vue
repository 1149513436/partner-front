...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('im.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> 新增 </span>
      </el-button>
      <el-upload
          v-if="auths.includes('im.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/im/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('im.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('im.deleteBatch')">
        <template #reference>
    ...
    <div style="margin: 10px 0">
      <el-table :data="state.tableData" stripe border  @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="img" label="聊天图片"></el-table-column>
        <el-table-column prop="sign" label="个性签名"></el-table-column>
        <el-table-column prop="text" label="聊天内容"></el-table-column>
        
        <el-table-column label="操作" width="180">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('im.edit')">编辑</el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('im.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="动态信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        ...
        <el-form-item prop="text" label="聊天内容">
          <el-input v-model="state.form.text" type="textarea" autocomplete="off" />
        </el-form-item>
