<template>
    <div class="main-body" >
        <DynamicScroller :items="state.fakerArr" @scroll="handleScroll" :min-item-size="60" class="scroller">
...
const handleScroll = () => {
    ...
    const items = listEl.querySelectorAll('.image-item')
    const visibleItems = []
    const scrollTop = listEl.scrollTop
    const clientHeight = listEl.clientHeight
    items.forEach((item) => {
        const rect = item.getBoundingClientRect()
        if (rect.top + rect.height >= scrollTop && rect.top <= scrollTop + clientHeight) {
