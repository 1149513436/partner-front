<template>
    <div class="main-body" >
        <DynamicScroller ref="list" :items="state.fakerArr" @scroll="handleScrollThrottled" :min-item-size="60" class="scroller">
...
const handleScroll = () => {
    ...
    console.log("观察者",observer.value);
    if (!observer.value) {
        console.log("真假",!observer.value);
        ...
        observer.value = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    observer.value.unobserve(imgt); // 停止观察已经加载的图片
                }
            });
        });
    }
    ...
    items.forEach(img => observer.value.observe(img));
}
